...
from gaussian_renderer_fov_mmfr import GaussianModel, render
...
def render_set(out_path, name, iteration, views, pipeline, background, multi_gs):
    ...
    for gi, gaze in enumerate(gaze_samples):
        ...
        logger.info(f"Average FPS of gaze {gaze[0]}, {gaze[1]}: {avg_fps}")
        avg_fpss.append(avg_fps)

    logger.info("Average FPS: ", sum(avg_fpss) / len(avg_fpss))
...
def render_sets(
    ...
    with torch.no_grad():
        ...
        scene = Scene(
            dataset, gaussians, load_iteration=None, shuffle=False, fps_mode=True
        )
        finest_gs_path = (
            args.base_folder
            + "/L0/1_PS1_4_12/point_cloud/iteration_55000/point_cloud.ply"
        )
        ...
        gaussians = GaussianModel(dataset.sh_degree)
        layer1_gs_path = (
            args.base_folder + "/L1/point_cloud/iteration_35000/point_cloud.ply"
        )
        ...
        gaussians = GaussianModel(dataset.sh_degree)
        layer2_gs_path = (
            args.base_folder + "/L2/point_cloud/iteration_35000/point_cloud.ply"
        )
        ...
        layer3_gs_path = (
            args.base_folder + "/L3/point_cloud/iteration_35000/point_cloud.ply"
        )
