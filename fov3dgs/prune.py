...
from odak.learn.perception import MetamericLossUniform
        ...
        with torch.no_grad():
            if args.metric == "max_comp_efficiency":
                ...
                render_pkg = render(
                    ...
                    pipe,
                    bg,
                    cuda_type="pcheck_obb_loss_weighted_max_count",
                    loss_map=loss_map,
                )
                ...
                render_pkg = render(
                    ...
                    pipe,
                    bg,
                    only_train_shs_dc=True,
                    cuda_type="pcheck_obb_max",
                )
                gs_count = render_pkg["gs_count"].unsqueeze(1).float()
                contibs = render_pkg["contribs"].unsqueeze(1).float()
                cur_metric = contibs
...
def test_hvs_loss(
    gaussians, scene=None, pipe=None, bg=None, need_resize=False, size=(256, 256)
):
    # test hvs loss in training set
    hvs_loss = MetamericLossUniform(
        n_pyramid_levels=args.n_pyramid_levels,
    ...
    for viewpoint_cam in view_stacks:
        with torch.no_grad():
            render_pkg = render(
                viewpoint_cam, gaussians, pipe, bg, cuda_type="pcheck_obb"
            )
...
def training(dataset, opt, pipe, debug_from, trial_name):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render(
            viewpoint_cam, gaussians, pipe, bg, cuda_type="pcheck_obb_sum"
        )
        image = render_pkg["render"]
        ...
        original_loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (
        ...
        with torch.no_grad():
            ...
            if (iteration % prune_interval == 1) and iteration < args.pruning_iters:
                ...
                # print point number
                logger.info(
                    "Iteration = {}, Point Number Before Pruning: {}".format(
                ...
                logger.info(
                    "Iteration = {}, Testing SSIM loss, SSIM Loss in training/test set: {}".format(
                        iteration, tested_ssim
                    )
                )
                logger.info(
                    "Iteration = {}, Testing PSNR loss, PSNR Loss in training/test set: {}".format(
                        iteration, tested_psnr
                    )
                )
                ...
                if pass_test:
                    logger.info(
                        "Iteration = {}, Pass Test, Saving Model Before Prune".format(
                            iteration
                        )
                    )
                    torch.save(
                        (gaussians.capture(), iteration),
                        scene.model_path + "/current-best.pth",
                    )
                    # perform metric pruning
                    for i in range(per_prune_iterative_times):
                        gaussians = metric_pruning(
                            gaussians,
                    ...
                    logger.info(
                        "Iteration = {}, Metric Pruning Done, Point Number After Pruning: {}".format(
                            iteration, gaussians.get_xyz.shape[0]
                        )
                    )
                    ...
                    logger.info(
                        "Iteration = {}, Scale Weight: {}".format(
                            iteration, scale_weight
                        )
                    )
                    ...
                    logger.info(
                        "Iteration = {}, Not Pass Test, Metric Pruning Skipped".format(
                            iteration
                        )
                    )
                    logger.info(
                ...
                if not args.use_scale_decay:
                    scale_weight = 0.0
                    logger.info(
                        "Iteration = {}, No Scale Weight: {}".format(
                            iteration, scale_weight
                        )
                    )
            ...
            if iteration == args.pruning_iters:
                ...
                pass_test = (tested_ssim >= args.target_ssim) & (
                    tested_psnr >= args.target_psnr
                )
                if not pass_test:
                    best_checkpoint = scene.model_path + "/current-best.pth"
                    if os.path.exists(best_checkpoint):
                        logger.info(
                            "Iteration = {}, Tested Loss Not Pass, Reload Best Model and Perform Final Prune Before Adatation: ".format(
                                iteration
                            )
                        )
                        ...
                        logger.info(
                            "Iteration = {}, Tested Loss Not Pass, but no best checkpoint found, continuing with current model: ".format(
                                iteration
                            )
                        )
                    # Save current as best before pruning
                    torch.save((gaussians.capture(), iteration), best_checkpoint)
                else:
                    logger.info(
                        "Iteration = {}, Pruning Done, Tested Loss Pass".format(
                            iteration
                        )
                    )
                ...
                for i in range(per_prune_iterative_times):
                    logger.info(
                        "Iteration = {}, Perform  {} th Final Prune Saving Model Before Prune ".format(
                            iteration, i
                        )
                    )
                    best_checkpoint = scene.model_path + "/current-best.pth"
                    ...
                    tested_ssim = test_ssim_loss(
                        gaussians, scene=scene, pipe=pipe, bg=bg
                    )
                    tested_psnr = test_psnr_loss(
                        gaussians, scene=scene, pipe=pipe, bg=bg
                    )
                    logger.info(
                        "Iteration = {}, Testing SSIM loss, SSIM Loss in training/test set: {}".format(
                    ...
                    if not pass_test:
                        logger.info(
                            "Iteration = {}, Final Pruning Meet target, Reload Best Model and leave for adaptation".format(
                                iteration
                            )
                        )
        ...
        with torch.no_grad():
            ...
            # Always save at final iteration
            if iteration >= opt.iterations - 1:
                gaussians.prune(prune_method="opacity", threshold=0.005)
                tested_ssim = test_ssim_loss(gaussians, scene=scene, pipe=pipe, bg=bg)
                tested_psnr = test_psnr_loss(gaussians, scene=scene, pipe=pipe, bg=bg)
                logger.info(
                    "Iteration = {}, Final Testing SSIM loss, SSIM Loss: {}".format(
                        iteration, tested_ssim
                    )
                )
                logger.info(
                    "Iteration = {}, Final Testing PSNR loss, PSNR Loss: {}".format(
                        iteration, tested_psnr
                    )
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pretrain_ply", type=str, default=None
    )  # pretrained 3dgs for pruning
    # Set up the HVS loss
    ...
    parser.add_argument(
        "--target_hvs", type=float, required=True
    )  # targeted loss for adaptive pruning
    parser.add_argument(
        "--target_ssim", type=float, required=True
    )  # targeted loss for adaptive pruning
    parser.add_argument(
        "--target_psnr", type=float, required=True
    )  # targeted loss for adaptive pruning
    parser.add_argument(
        "--pruning_iters", type=int, default=25000
    )  # iterations for pruning
    parser.add_argument(
        "--final_adaptation_iters", type=int, default=5000
    )  # final adaptation iterations number after pruning
    parser.add_argument(
        "--metric",
        type=str,
        default="max_comp_efficiency",
        choices=["max_comp_efficiency", "max_contrib", "surface"],
    )

    # Other arguments
    parser.add_argument(
        "--trial_name", type=str, default=None
    )  # working directory = dataset_path +/ trial_name
    parser.add_argument(
        "--position_lr_init_scale", type=float, default=0.1
    )  # scale the initial learning rate for position
