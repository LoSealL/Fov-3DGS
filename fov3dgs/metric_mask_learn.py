    ...
    for viewpoint_cam in view_stacks:
        with torch.no_grad():
            if args.metric == "max_comp_efficiency":
                ...
                render_pkg = render(
                    viewpoint_cam, gaussians, pipe, bg, cuda_type="pcheck_obb_max"
                )
                gs_count = render_pkg["gs_count"].unsqueeze(1).float()
                contibs = render_pkg["contribs"].unsqueeze(1).float()
                cur_metric = contibs
...
def training(dataset, opt, pipe, debug_from, trial_name):
    uniform_hvs_loss = MetamericLossUniform(
        n_pyramid_levels=args.n_pyramid_levels,
        n_orientations=args.n_orientations,
        pooling_size=args.pooling_size,
        device="cuda",
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render(
            viewpoint_cam, gaussians, pipe, bg, masking=True, cuda_type="pcheck_obb_sum"
        )
        image = render_pkg["render"]
        ...
        if need_resize:
            gt_image = F.interpolate(
                gt_image.unsqueeze(0),
                size=(required_height, required_width),
                mode="bilinear",
                align_corners=False,
            )
            image = F.interpolate(
        ...
        with torch.no_grad():
            ...
            if iteration < args.pruning_iters:
                ...
                # print point number
                (
                    "Iteration = {}, Point Number Before Masking: {}".format(
                        iteration, gaussians.get_xyz.shape[0]
                    ),
                    log_path,
                )
                # perform metric Masking
                tested_loss = test_hvs_loss(
                    gaussians,
                    scene=scene,
                    pipe=pipe,
                    bg=bg,
                ...
                logger.info(
                    "Iteration = {}, Testing HVS loss, HVS Loss in training set: {}".format(
                        iteration, tested_loss
                    )
                )
                ...
                if tested_loss <= args.target_loss:
                    ...
                    logger.info(
                        "Iteration = {}, Pass Test, Saving Model Before Masking".format(
                            iteration
                        )
                    )
                    torch.save(
                    ...
                    logger.info(
                        "Iteration = {}, Metric Pruning Done, Point Number After Masking: {}".format(
                            iteration, gaussians.get_xyz.shape[0]
                        )
                    )
                    ...
                    logger.info(
                        "Iteration = {}, Not Pass Test, Metric Masking Skipped".format(
                            iteration
                        )
                    )
            ...
            if iteration == args.pruning_iters:
                ...
                logger.info(
                    "Iteration = {}, Masking Done, Testing HVS loss, HVS Loss in training set: {}".format(
                        iteration, tested_loss
                    )
                )
                ...
                if tested_loss > args.target_loss:
                    ...
                    if os.path.exists(best_checkpoint):
                        logger.info(
                            "Iteration = {}, Tested Loss Not Pass, Reload Best Model Before Final Pruning: ".format(
                                iteration
                            )
                        )
                ...
                for i in range(per_prune_iterative_times):
                    logger.info(
                        "Iteration = {}, Final Prune {} Before Adaptation, Saving Model Before Prune".format(
                            iteration, i
                        )
                    )
                    torch.save(
                    ...
                    if tested_loss > args.target_loss:
                        logger.info(
                            "Iteration = {}, Final Prune {}  Meet Target, Reload Best Model Before Adatation: ".format(
                                iteration, i
                            )
                        )
                        best_checkpoint = scene.model_path + "/current-best.pth"
        ...
        with torch.no_grad():
            ...
            if iteration >= opt.iterations - 1:
                ...
                logger.info(
                    "Iteration = {}, Final Testing HVS loss, HVS Loss in training set: {}".format(
                        iteration, tested_loss
                    )
                )
...
if __name__ == "__main__":
    ...
    # Set up the HVS loss
    parser.add_argument(
        "--hvs_loss_type", type=str, default="L1"
    )  # what loss to use during pruning
    ...
    parser.add_argument(
        "--target_loss", type=float, default=0.5
    ...
    parser.add_argument(
        "--init_index", action="store_true", default=False
    )  # whether to initialize the index for the 3DGS, it is for FOV masking
