        ...
        try:
            ...
            for method in os.listdir(test_dir):
                ...
                for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                    ...
                    hvs_uniforms.append(
                        hvs_calc.calc_uniform_loss(renders[idx], gts[idx])
                    )
                    hvs_fovs.append(hvs_calc.calc_fov_loss(renders[idx], gts[idx]))
                    # import ipdb; ipdb.set_trace()

                logger.info(
                    "  SSIM : {:>12.7f}".format(torch.tensor(ssims).mean(), )
                )
                logger.info(
                    "  PSNR : {:>12.7f}".format(torch.tensor(psnrs).mean(), )
                )
                logger.info(
                    "  LPIPS: {:>12.7f}".format(torch.tensor(lpipss).mean(), )
                )
                logger.info(
                    "  HVS Uniform: {:>12.7f}".format(
                        torch.tensor(hvs_uniforms).mean(), )
                )
                logger.info(
                    "  HVS FOV    : {:>12.7f}".format(
                        torch.tensor(hvs_fovs).mean(), )
                )
                logger.info("")
                ...
                per_view_dict[scene_dir][method].update(
                    {
                        ...
                        "HVS Uniform": {
                            name: hvs
                            for hvs, name in zip(
                                torch.tensor(hvs_uniforms).tolist(), image_names
                            )
                        },
                        "HVS FOV": {
                            name: hvs
                            for hvs, name in zip(
                                torch.tensor(hvs_fovs).tolist(), image_names
                            )
                        },
                    }
                )
            ...
            logger.info("Unable to compute metrics for model", scene_dir)
