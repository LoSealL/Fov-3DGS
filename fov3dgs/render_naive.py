...
def render_set(
    model_path, name, iteration, views, gaussians, pipeline, background, scene=None
):
    render_path = os.path.join(
        args.out_folder, name, "ours_{}".format(iteration), "renders"
    )
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        for i in range(frame_num):
            rendering = render(
                ...
                starter=starter,
                ender=ender,
                cuda_type="pcheck_obb",
            )["render"]
            torch.cuda.synchronize()
            time += starter.elapsed_time(ender)

        fps = frame_num / (time / 1000)
        logger.info("FPS: ", frame_num / (time / 1000))
    ...
    logger.info("Average FPS: ", sum(fps_list) / len(fps_list))
