...
def run_masking_command(
    ...
    if init_idx:
        base_command.append("--init_index")

    if args.monitor_val:
        base_command.append("--monitor_val")

    # Complete command with additional flags
    logger.info(f"Running command: {' '.join(base_command)}")
    # import ipdb; ipdb.set_trace()
    result = subprocess.check_output(base_command, executable=sys.executable)
...
def main():
    ...
    parser.add_argument(
        "--budget", type=int, help="Budget for pruning, total iteration number"
    )
    ...
    if not os.path.exists(args.original_train_loss):
        # if not, run:
        if args.monitor_val:
            cmd = f" render.py -s {args.scene} -m {args.path_to_pretrain_trial}  --eval --iteration {args.ps1_iterations} --skip_train"
        else:
            cmd = f" render.py -s {args.scene} -m {args.path_to_pretrain_trial}  --eval --iteration {args.ps1_iterations} --skip_test"
        logger.info(
            "Original training loss txt not found, running render.py to generate it."
        )
        logger.info(cmd)
        subprocess.run(cmd, executable=sys.executable)
        logger.info(
            "Original training loss txt not found, running hvs_metric to generate it."
        )
        if args.monitor_val:
            cmd = f" hvs_metrics.py -m {args.path_to_pretrain_trial} -s test"
        else:
            cmd = f" hvs_metrics.py -m {args.path_to_pretrain_trial} -s train"
        subprocess.run(cmd, executable=sys.executable)
    ...
    full_folder_path, folder_name = create_folder(
        args.scene,
        args.layernum,
        args.max_pooling_size,
        args.target_loss_scale,
        args.ps1_loss_scale,
    )
    ...
    for i in range(args.layernum):
        ...
        if is_first:
            ...
            if not os.path.exists(target_trial_name):
                logger.info(
                    f"Copying from {args.path_to_ps1_trial} to {target_trial_name}"
                )
                shutil.copytree(args.path_to_ps1_trial, target_trial_name)
            # Copy point cloud if it doesn't exist
            pretrain_ply = f"{target_trial_name}/point_cloud/iteration_{args.ps1_iterations}/point_cloud.ply"
            if not os.path.exists(pretrain_ply):
                src_ply = f"{args.path_to_ps1_trial}/point_cloud/iteration_{args.ps1_iterations}/point_cloud.ply"
                if os.path.exists(src_ply):
                    os.makedirs(os.path.dirname(pretrain_ply), exist_ok=True)
                    shutil.copy2(src_ply, pretrain_ply)
                    logger.info(f"Copied point cloud from {src_ply} to {pretrain_ply}")
        else:
            ...
            output_ply = f"{args.scene}/{trial_name}/point_cloud/iteration_{prune_iters + adaptation_iters}/point_cloud.ply"
