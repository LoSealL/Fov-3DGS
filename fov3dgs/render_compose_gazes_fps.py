...
def render_sets(
    ...
    with torch.no_grad():
        ...
        finest_gs_path = (
            args.base_folder
            + f"1_PS1_{args.layer_num}_{args.max_pooling_size}/point_cloud/iteration_30002/point_cloud.ply"
        )
        ...
        mask_path = os.path.join(
            args.base_folder,
            f"composed_{args.layer_num}_{args.max_pooling_size}",
            "highest_levels.pt",
        )
        highest_levels = torch.load(mask_path)
        shs_dcs_path = os.path.join(
            args.base_folder,
            f"composed_{args.layer_num}_{args.max_pooling_size}",
            "shs_dcs.pt",
        )
        shs_dcs = torch.load(shs_dcs_path)
        opacities_path = os.path.join(
            args.base_folder,
            f"composed_{args.layer_num}_{args.max_pooling_size}",
            "opacities.pt",
        )
        ...
        out_path = os.path.join(
            args.base_folder, f"composed_{args.layer_num}_{args.max_pooling_size}"
        )
